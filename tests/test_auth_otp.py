...
class TestRequestOTP:
    def test_registered_user_receives_a_code(self, staff):
        challenge = OTPService().request_otp(staff.phone_number)

        assert challenge.phone_number == staff.phone_number
        assert len(InMemorySMSBackend.outbox) == 1

    def test_code_is_never_stored_in_plain_text(self, staff):
        OTPService().request_otp(staff.phone_number)
        code = sent_code()
    ...
    def test_disabled_account_cannot_request(self, staff):
    ...
    def test_resend_is_blocked_during_cooldown(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)

        with pytest.raises(RateLimited) as exc:
            service.request_otp(staff.phone_number, is_resend=True)
        assert "retry_after_seconds" in exc.value.details

    def test_daily_cap_is_enforced(self, staff, settings):
        settings.OTP_SETTINGS = {**settings.OTP_SETTINGS, "MAX_SENDS_PER_DAY": 2}
        service = OTPService()

        service.request_otp(staff.phone_number)
        cache.delete(service._cooldown_key(staff.phone_number))
        service.request_otp(staff.phone_number)
        cache.delete(service._cooldown_key(staff.phone_number))
    ...
    def test_issuing_a_new_code_kills_the_previous_one(self, staff, settings):
        ...
        with pytest.raises(ValidationFailed):
            service.verify(staff.phone_number, first_code)


class TestVerifyOTP:
    def test_correct_code_returns_the_user(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)

        assert service.verify(staff.phone_number, sent_code()) == staff

    def test_code_cannot_be_replayed(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)
        code = sent_code()

        service.verify(staff.phone_number, code)
        with pytest.raises(ValidationFailed):
            service.verify(staff.phone_number, code)

    def test_wrong_code_is_rejected_and_counted(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)

        with pytest.raises(ValidationFailed):
            service.verify(staff.phone_number, "000000")

        assert OTPRequest.objects.latest("created_at").attempts == 1

    def test_expired_code_is_rejected(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)
        code = sent_code()
    ...
    def test_repeated_failures_lock_the_number_out(self, staff, settings):
        ...
        for _ in range(3):
            with pytest.raises((ValidationFailed, RateLimited)):
                service.verify(staff.phone_number, "111111")
        ...
        with pytest.raises(RateLimited):
            service.verify(staff.phone_number, sent_code())

    def test_non_numeric_input_is_rejected(self, staff):
        service = OTPService()
        service.request_otp(staff.phone_number)

        with pytest.raises(ValidationFailed):
            service.verify(staff.phone_number, "abcdef")


class TestTokens:
    def test_login_returns_tokens_and_registers_the_device(self, staff):
        auth = AuthService()
        auth.otp.request_otp(staff.phone_number)
    ...
    def test_login_works_without_a_device_id(self, staff):
        """Device tracking is a convenience, not a requirement."""
        auth = AuthService()
        auth.otp.request_otp(staff.phone_number)

        user, device, tokens = auth.login_with_otp(
            phone_number=staff.phone_number, code=sent_code()
        )
        assert user == staff
        assert device is None
        assert tokens.access

    def test_refresh_rotates_and_burns_the_old_token(self, staff):
        auth = AuthService()
        tokens = auth.issue_tokens(staff)
    ...
    def test_refresh_fails_once_the_account_is_disabled(self, staff):
        auth = AuthService()
        tokens = auth.issue_tokens(staff)
...
class TestAuthEndpoints:
    def test_full_login_round_trip(self, api_client, staff):
        ...
        assert response.data["data"]["user"]["role"] == "staff"

    def test_any_common_phone_format_works(self, api_client, staff):
    ...
    def test_me_returns_the_current_user(self, staff_client, staff):
        response = staff_client.get("/api/v1/auth/me/")
        assert response.status_code == 200
        assert response.data["data"]["phone_number"] == staff.phone_number
        assert response.data["data"]["can_contribute"] is True

    def test_user_can_edit_their_own_name(self, staff_client):
    ...
    def test_user_cannot_promote_themselves(self, staff_client):
        """Self-service escalation is the classic hole in a simple role model."""
        staff_client.patch("/api/v1/auth/me/", {"role": "owner"}, format="json")

        response = staff_client.get("/api/v1/auth/me/")
        assert response.data["data"]["role"] == "staff"

    def test_logout_succeeds(self, staff_client):
        assert staff_client.post("/api/v1/auth/logout/", {}, format="json").status_code == 200
...
class TestCreateUserCommand:
    ...
    def test_creates_an_account(self, db):
        ...
        call_command("create_user", "--phone", "9876543210", "--name", "Owner", "--role", "owner")
    ...
    def test_rejects_a_duplicate_without_update(self, db, staff):
        from django.core.management import call_command
        from django.core.management.base import CommandError

        with pytest.raises(CommandError):
            call_command("create_user", "--phone", staff.phone_number, "--name", "Clone")

    def test_updates_a_role(self, db, staff):
        from django.core.management import call_command

        call_command("create_user", "--phone", staff.phone_number, "--role", "viewer", "--update")

        staff.refresh_from_db()
        assert staff.role == "viewer"
        assert not staff.can_contribute

    def test_disables_and_reenables(self, db, staff):
        from django.core.management import call_command

        call_command("create_user", "--phone", staff.phone_number, "--disable")
        staff.refresh_from_db()
        assert not staff.is_active

        call_command("create_user", "--phone", staff.phone_number, "--enable")
        staff.refresh_from_db()
